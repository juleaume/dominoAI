    ...
    while nbGame<10000
        ...
        while game==1 %while the game is on
            ...
            if turnOrder==1
                ...
                token=selectTokenTree(ch, fringe);
            else
...
function index = randomToken(h,f) %returns a random token from the hand. For testing only
d=initTiles();
indexa=[];
index=0;
if f(1)~=-1
...
function index=selectTokenTree(h, fringe)
csfpa=[]; %Cost So Far Array
ph=[]; %Pairs in the Hand
[d,p]=initTiles(); %dominoes and Pairs
for i=1:size(h,2) %for each domino in the hand
    if h(i)==p %if the domino is a pair
        ph=[ph,i]; %add it to the list
    end %end if
end %end for
if size(ph,2)~=0 && (fringe(1)==-1 && fringe(2)==-1) %If there are some pairs in the hand AND the fringe is void
    index=max(ph); %Return the highest pair
else %else if there are no pairs or the fringe is not empty
    side=1; %only look at one side of the fringe
    if fringe(1)~=fringe(2) %if each side is different
        side=2; %look at both
    end %end if
    for i=1:side %for each side of the fringe we look at
        csfp=selectNext(h, fringe(i), 0,0); %Cost So Far Path is asked
        lencsfpa=size(csfpa,1); %the length of the new csfp is recorded
        for j=1:size(csfp,2) %for each line of the new csfp
            for k=1:size(csfp,1) %for each element in the path
                csfpa(lencsfpa+k,j)=csfp(k,j); %add it to the Cost So Far Path Array
            end %we use this method  because of concatenation errors
        end
    end
    dont=0;
    for i=size(csfpa,1)
    ...
    if dont==0
        [nm,inm]=max(csfpa); %we record the highest score and its positon
        if size(inm,2)==1
            inm=1;
        end
        isoscore=csfpa(inm(1),2); %set the isoscore as the best found
        for i=1:size(csfpa,1) %for each paths found
            if csfpa(i,1)==nm(1) && i~=inm(1) %if a path has the same score as the highest
                isoscore=[isoscore;csfpa(i,2)]; %add it to the isoscores
            end
        end
        if size(isoscore,1)==1 %if there is only one isoscore
            index=csfpa(inm(1),2); %return it
        else %else
            maxdom=0; %set the highest domino as 0
            imaxdom=1;
            for i=1:size(isoscore,1) %for each highest paths
                if csfpa(i,2)~=0
                    stt=d(h(csfpa(i,2)),1)+d(h(csfpa(i,2)),2);%score this turn
                    if stt>maxdom %if the Score This Turn is higher than the previous
                        maxdom=stt; %set it as the highest of the highest
                        imaxdom=i; %record the index
                    end
                end
            end
            index=isoscore(imaxdom); %return the index of the highest score this turn
        end
    else
        index=0;
    end
...
function csfpath=selectNext(hand, f, csf, path)
d=initTiles();
csfpath=[csf,path];
for i=1:size(hand,2) %for all remaining dominoes
    iDom=hand(i); %index of the domino selected
    if iDom~=0 %if the domino has not been used yet
        for j=1:2 %for each side of the domino
            if (d(iDom,j)==f || f==-1) && path(size(path,1),size(path,2))~=i %if the domino can be played
                csf=csf+d(iDom,1)+d(iDom,2); %we add the cost;
                if path==0 %if this is the first of the path
                    path=i; %we start a new path
                else %otherwise
                    path=[path, i]; %we add it to the list
                end
                hand(i)=0; %remove but keep the index position
                csfpath=[csf, path]; %update the csfpath
                last=1; %bool setup
                for k=1:size(hand,2) %for each remaining dominoes of the hand
                    if hand(k)~=0 %if a domino exists
                        last=0; %this was not the last one
                        break %we exit the for loop (don't need to continue)
                    end
                end
                if last==0 %if it was not the last domino
                    newpath=selectNext(hand, d(iDom,mod(j,2)+1), csf, path); %create a new path with recuring algo
                    dont=0;
                    if size(newpath,1)==size(csfpath,1)&&size(newpath,2)==size(csfpath,2)
                        if newpath(:)==csfpath(:)
                            dont=1;
                        end
                    end
                    if dont==0
                        lencsf=size(csfpath,1); %set the size of the Cost So Far
                        %We use a double for method because the size of the previous array
                        %might be too small to concatenate.
                        %csfpath=[csfpath;newpath];
                        for k=1:size(newpath,2) %for each path in the matrix
                            for line=1:size(newpath,1) %for each domino in the current path
                                csfpath(lencsf+line,k)=newpath(line,k);%we start again with the new hand
                            end
                        end
                    end
                else %if it was the last domino of the hand
                    return %we return the arrays
                end
            end
        end
    end
